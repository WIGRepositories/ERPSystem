...
var mycrtl1 = myapp1.controller('myCtrl', function ($scope, $http, $localStorage, $uibModal, $filter, fileReader) {
    ...
    $scope.getJobDetails = function (selJobId) {
        ...
        $http.get('/api/RFQ/GetRFQDetails?rfqId=' + selJobId).then(function (res, data) {
            $scope.currJob = res.data.Table[0];
    ...
    $scope.GetUsersForRFQ = function () {
        $http.get('/api/RFQ/GetUsersForRFQ?jobId=' + $scope.j.ID).then(function (res, data) {
    ...
    $scope.GetAssetsModels = function () {
        $http.get('/api/RFQ/GetItemModels').then(function (res, data) {
            $scope.Models = res.data;
        });        
    }

    $scope.GetAssets = function () {
        var amd = ($scope.amid == null) ? 1 :$scope.amid.ID;
        //var locId = ($scope.currJob.LocationID == null) ? -1 : $scope.currJob.LocationID;
        $http.get('/api/RFQ/GetItemsForRFQ?modelId=' + amd + '&rfqId=' + $scope.currJob.ID).then(function (res, data) {
